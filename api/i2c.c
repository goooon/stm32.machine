...
void I2C1_init(void)
{
	...
	/*
      ...
      （2）GPIO_Mode_IN_FLOATING 浮空输入  ——浮空输入，可以做KEY识别，RX1
	  （3）GPIO_Mode_IPD 下拉输入 —— IO内部下拉电阻输入 
 	  （4）GPIO_Mode_IPU 上拉输入 ——IO内部上拉电阻输入
	  （5）GPIO_Mode_Out_OD 开漏输出  ——IO输出0接GND，IO输出1，悬空，需要外接上拉电阻，才能实现输出高电平。当输出为1时，
	                                    IO口的状态由上拉电阻拉高电平，但由于是开漏输出模式，这样IO口也就可以由外部电路改变为低电平或不变。
	...
	*/
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF_OD;  //工作模式
    GPIO_InitStruct.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7;//引脚号
    GPIO_InitStruct.GPIO_Speed = GPIO_Speed_10MHz;  //配置速度
    GPIO_Init(GPIOB,&GPIO_InitStruct);

    I2C_InitStruct.I2C_Mode = I2C_Mode_I2C;/* 指定工作模式,可选I2C模式和SMBUS模式*/
    I2C_InitStruct.I2C_Ack = I2C_Ack_Enable;/*使能或者关闭响应 (一般都是使能) */
    I2C_InitStruct.I2C_ClockSpeed = 100000;/*设置SCL时钟频,此值不低于40000*/
    I2C_InitStruct.I2C_DutyCycle = I2C_DutyCycle_2;/*指定时钟占空比,可选 low/high=2:1以及16:9模式*/
    I2C_InitStruct.I2C_OwnAddress1 = 0xa0;/*指定自身的I2C设备地址 */
    I2C_InitStruct.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit;/*指定地址的长度,可以为7位及10位 */
